...
const softwareOpts = {
  software: {
    ...
    licenseKey: "GIPUZ-LIC-001",
  },
};

describe("buildGipuzkoaXml", () => {
  it("produces valid XML string", () => {
    const xml = buildGipuzkoaXml(testInvoice, softwareOpts);
  ...
  it("includes previous hash in Encadenamiento when provided", () => {
    const xml = buildGipuzkoaXml(testInvoice, { ...softwareOpts, previousHash: "abc123" });
    expect(xml).toContain("Encadenamiento");
  ...
  it("is deterministic", () => {
    expect(buildGipuzkoaXml(testInvoice, softwareOpts)).toBe(buildGipuzkoaXml(testInvoice, softwareOpts));
  });
});
